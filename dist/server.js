    ...
    inquire
        ...
        switch (answers.mainQuery) {
            case 'View All Employees':
                ...
                break;
            case 'View All Roles':
                pool.query('SELECT * FROM depto_role', (err, result) => {
                    if (err) {
                        console.log(err);
                    }
                    else if (result) {
...
function addDepartment() {
    inquire
        ...
        .then((answers) => {
        pool.query(`INSERT INTO departments(name) VALUES ('${answers.newDepto}');`, (err, result) => {
            if (err) {
                console.log(err);
            }
            else if (result) {
    ...
    pool.query(`SELECT * FROM departments`, (err, result) => {
        if (err) {
            ...
            inquire
                ...
                .then((answers) => {
                pool.query(`INSERT INTO depto_role(title, salary, department_id) VALUES ('${answers.title}', ${parseInt(answers.salary)}, ${parseInt(answers.deptos)})`, (err, result) => {
                    if (err) {
                        console.log(err);
                    }
                    else if (result) {
            ...
            pool.query(`SELECT * FROM depto_role`, (err, result) => {
                if (err) {
                    ...
                    inquire
                        ...
                        .then((answers) => {
                        pool.query(`UPDATE employees SET role_id = ${answers.role} WHERE id = ${answers.employee};`, (err, result) => {
                            if (err) {
                                console.log(err);
                            }
                            else if (result) {
